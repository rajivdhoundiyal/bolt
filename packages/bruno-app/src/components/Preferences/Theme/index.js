...
const Theme = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      theme: Yup.string().oneOf(['light', 'dark', 'vibrant']).required('theme is required')
    }),
    onSubmit: (values) => {
      ...
      <div className="bruno-form">
        <div className="flex items-center mt-2">
          ...
          <input
            ...
            value="vibrant"
            checked={formik.values.theme === 'vibrant'}
          />
          <label htmlFor="vibrant-theme" className="ml-1 cursor-pointer select-none">
            Vibrant
          </label>
        </div>
      </div>
